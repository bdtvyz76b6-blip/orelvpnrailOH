...
def encrypt_happ_crypt4(subscription_url):
    binary = find_hpwnr()
    if not binary:
        raise RuntimeError("hpwnr binary not found")
...
def render_page(
    ...
    days = get_days_left(subscription_until)
    name = safe_text(
        first_name,
        "Пользователь",
    )
...
.logo{{
    ...
    background:linear-gradient(145deg,#252527,#0d0d0e);
...
def subscription(token):
    ...
    try:
        ...
        # Если обновление не удалось, всё равно
        # пытаемся отдать уже сохранённый контент.
        try:
