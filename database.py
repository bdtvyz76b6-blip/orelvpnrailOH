        ...
        with conn.cursor() as cur:
            cur.execute(
                ...
                (
                    ...
                    username,
                    first_name,
                    get_canonical_subscription_link(user_id),
                ),
            )
            conn.commit()
    except Exception:
...
def get_subscription_link(
    ...
    return get_canonical_subscription_link(user_id)
# =========================================================
...
def check_trial(
    ...
    if not user:
        return False
    return bool(
        user.get("trial_used", False)
    )
def trial_used(
    user_id: int,
) -> bool:
    return check_trial(user_id)
def mark_trial_used(
    user_id: int,
):
    create_user(user_id)
...
def use_promocode(
    ...
    try:
        with conn.cursor(
            ...
            return (
                True,
                f"✅ Промокод применён! Начислено {days} дн.",
                days,
            )
    except Exception:
        conn.rollback()
...
def create_payment(
    ...
    try:
        with conn.cursor() as cur:
            # Защита от повторной записи одного платежа.
            if payment_id:
                cur.execute(
                    """
                    SELECT id
    ...
    try:
        with conn.cursor(
            ...
            if not payment:
                conn.rollback()
                return None
            # Уже обработан — повторно дни не начисляем.
...
def has_accepted_terms(
    ...
    return get_accepted_terms(user_id)
