...
# =========================================================
def save_subscription_content(user_id, content):
    conn = connect()
    cur = conn.cursor()
    try:
        cur.execute("""
...
# =========================================================
def calculate_subscription_date(current_until, days):
    days = int(days)
    if days < 1:
        raise ValueError(
...
def activate_subscription(user_id, link, days):
    ...
    try:
        ...
        """, (user_id,))
        user = cur.fetchone()
        current_until = user[0] if user else ""
...
# ИСПОЛЬЗОВАЛ ЛИ
# =========================================================
def has_used_promocode(user_id, code):
    code = str(code).strip().upper()
    conn = connect()
    cur = conn.cursor()
    try:
...
def extend_subscription(user_id, days):
    ...
    try:
        ...
        """, (user_id,))
        result = cur.fetchone()
        current_until = result[0] if result else ""
        new_date = calculate_subscription_date(
            current_until,
            days
        )
