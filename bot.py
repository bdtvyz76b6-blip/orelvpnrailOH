...
<script>
const subscriptionUrl = '{escaped_subscription}';
...
def cashera():
    ...
    payment_uuid = transaction.get("uuid")
...
# IXXY CODES API
# =========================================================
@app.route("/add-days", methods=["POST"])
def add_days_api():
    data = request.get_json(silent=True)
    if not data:
        return {
    ...
    try:
        ...
        return {
            "status": "ok",
            "date": new_date,
            "subscription": make_subscription_link(
                user_id
            )
        }
    except Exception as e:
